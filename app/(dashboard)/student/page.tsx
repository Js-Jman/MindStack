...
//       try {
//         setIsLoadingStats(true);
//         const res = await fetch(`/api/stats?studentId=${user.id}`);
...
//       fetch(`/api/stats?studentId=${user.id}`),
//     ]);
//     if (cRes.ok) {
...
//               <StatsCard title="Completed Courses" value={stats.completedCourses} icon="completed" />
//               <StatsCard title="In Progress" value={stats.inProgressCourses} icon="inProgress" />
//               <StatsCard title="Average Progress" value={`${stats.averageProgress}%`} icon="average" />
//             </div>
//           )}
//         </div>
...
export default function StudentDashboard() {
  ...
  // Get the student ID — fall back to nothing until auth is ready
